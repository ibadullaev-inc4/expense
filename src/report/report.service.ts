...
export class ReportService {
  ...
  getReportById(type: ReportType, id: string): ReportResponseDto {
    const report = data.
      report.filter(report => report.type === type ).filter(
      report => report.id === id )[0];
    if(!report) return
  ...
  updateReport(type: ReportType, id: string, body: UpdateReport): ReportResponseDto {
    const reportToUpdate =  data.report.filter(report => report.type === type ).filter(report => report.id === id );
    if(!reportToUpdate) return
    const reportIndex = data.report.findIndex(report => report.id === reportToUpdate[0].id);
